...
public:
    ...
    T numberOfNodes() const { return n_; }

    T numberOfEdges() const { return m_; }

    T degree(T u) const { return indexArray_[u + 1] - indexArray_[u]; }
    ...
    std::ranges::iota_view<T, T> nodeRange() const { return {0, n_}; }

    auto edgeRange() const {
        return edgeArray_ | std::views::filter([](auto e) { return e.u < e.v; });
    }

    const std::vector<WeightedEdge<T>> &edgeArray() const { return edgeArray_; }

    const std::vector<T> &edgeIDmapping() const { return edgeIDmapping_; }
