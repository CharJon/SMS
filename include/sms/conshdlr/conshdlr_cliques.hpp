#ifndef SMS_CONSHDLRCLIQUES_HPP
#define SMS_CONSHDLRCLIQUES_HPP
...
/** C++ constraint handler for cliques (on edge variables) */
class ConshdlrCliques : public scip::ObjConshdlr {

private:
    const NetworKit::Graph &sepGraph_;
    CliqueSeparator cliqueSeparator_;

    // config
    SCIP_Bool sepaCutpool_;
    int subCliqueSizeUpperBound_;
    std::vector<int> fails_;

public:
    /** constructor */
    explicit ConshdlrCliques(Scip *scip, const NetworKit::Graph &graph);

    /** destructor */
    ~ConshdlrCliques() override = default;
     ...
     *  SCIPaddVarLocksType(scip, ..., SCIP_LOCKTYPE_MODEL, nlockspos + nlocksneg, nlockspos + nlocksneg) on all
     * variables, since rounding in both directions of each variable can destroy both the feasibility of the constraint
     * and it's negation "3x -5y +2z < 2  or  3x -5y +2z > 7".
     *
     ...
     *    the constraint's negation (i.e. feasibility of the constraint), and infeasibility of c's negation (i.e.
     ...
     *  constraint handler should call SCIPaddVarLocksType(scip, y, SCIP_LOCKTYPE_MODEL, nlockspos + nlocksneg,
     * nlockspos + nlocksneg) and SCIPaddConsLocks(scip, c, nlockspos + nlocksneg, nlockspos + nlocksneg), because any
     * modification to the value of y or to the feasibility of c can alter the feasibility of the equivalence
     * constraint.
     */
...
private:
    SCIP_RETCODE sepaCliquesLp(SCIP *scip,              /**< SCIP data structure */
    ...
    SCIP_RETCODE sepaOneCliqueLp(SCIP *scip, SCIP_CONSHDLR *conshdlr, SCIP_RESULT *result,
