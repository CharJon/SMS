...
private:
    NetworKit::Graph const *const sepGraph_;
    sms::HoleSeparator holeSeparator_;
    SCIP_Bool sepaCutpool_;
    SCIP_Real minEfficacy_;

public:
    /** constructor */
    explicit ConshdlrHoles(Scip *scip, const NetworKit::Graph *sepGraph);
     ...
     *  - SCIP_BRANCHED   : no changes were made to the problem, but a branching was applied to resolve an
     * infeasibility
     ...
     *  - SCIP_BRANCHED   : no changes were made to the problem, but a branching was applied to resolve an
     * infeasibility
     *  - SCIP_SOLVELP    : at least one constraint is infeasible, and this can only be resolved by solving the
     * SCIP_LP
     *  - SCIP_INFEASIBLE : at least one constraint is infeasible, but it was not resolved
     *  - SCIP_FEASIBLE   : all constraints of the handler are feasible
     ...
     *  linear constraint handler should call SCIPaddVarLocksType(scip, x, SCIP_LOCKTYPE_MODEL, nlocksneg,
     * nlockspos), SCIPaddVarLocksType(scip, y, SCIP_LOCKTYPE_MODEL, nlockspos, nlocksneg) and
     *  SCIPaddVarLocksType(scip, z, SCIP_LOCKTYPE_MODEL, nlocksneg, nlockspos) to tell SCIP,
     *  that rounding up of x and z and rounding down of y can destroy the feasibility of the constraint, while
     * rounding down of x and z and rounding up of y can destroy the feasibility of the constraint's negation "3x
     * -5y +2z > 7". A linear constraint "2 <= 3x -5y +2z <= 7" should call SCIPaddVarLocksType(scip, ...,
     * SCIP_LOCKTYPE_MODEL, nlockspos + nlocksneg, nlockspos + nlocksneg) on all variables, since rounding in both
     * directions of each variable can destroy both the feasibility of the constraint and it's negation "3x -5y +2z
     * < 2  or  3x -5y +2z > 7".
     *
     *  If the constraint itself contains other constraints as sub constraints (e.g. the "or" constraint
     * concatenation "c(x) or d(x)"), the rounding lock methods of these constraints should be called in a proper
     * way.
     *  - If the constraint may get violated by the violation of the sub constraint c, it should call
     *    SCIPaddConsLocks(scip, c, nlockspos, nlocksneg), saying that infeasibility of c may lead to infeasibility
     * of the (positive) constraint, and infeasibility of c's negation (i.e. feasibility of c) may lead to
     * infeasibility of the constraint's negation (i.e. feasibility of the constraint).
     *  - If the constraint may get violated by the feasibility of the sub constraint c, it should call
     *    SCIPaddConsLocks(scip, c, nlocksneg, nlockspos), saying that infeasibility of c may lead to infeasibility
     * of the constraint's negation (i.e. feasibility of the constraint), and infeasibility of c's negation (i.e.
     * feasibility of c) may lead to infeasibility of the (positive) constraint.
     *  - If the constraint may get violated by any change in the feasibility of the sub constraint c, it should
     * call SCIPaddConsLocks(scip, c, nlockspos + nlocksneg, nlockspos + nlocksneg).
     *
     *  Consider the or concatenation "c(x) or d(x)". The variable rounding lock method of the or constraint handler
     *  should call SCIPaddConsLocks(scip, c, nlockspos, nlocksneg) and SCIPaddConsLocks(scip, d, nlockspos,
     ...
     *  constraint demands, that y == 1 if and only if c(x) is satisfied. The variable lock method of the
     * corresponding constraint handler should call SCIPaddVarLocksType(scip, y, SCIP_LOCKTYPE_MODEL, nlockspos +
     * nlocksneg, nlockspos + nlocksneg) and SCIPaddConsLocks(scip, c, nlockspos + nlocksneg, nlockspos +
     * nlocksneg), because any modification to the value of y or to the feasibility of c can alter the feasibility
     ...
     *
     *  Separates all constraints of the constraint handler. The method is called outside the LP solution loop
     * (e.g., by a relaxator or a primal heuristic), which means that there is no valid LP solution. Instead, the
     ...
     *  - SCIP_REDUCEDDOM : a variable's domain was reduced
     *  - SCIP_SEPARATED  : a cutting plane was generated
     *  - SCIP_DIDNOTFIND : the separator searched, but did not find domain reductions, cutting planes, or cut
     * constraints
...
private:
    ...
    SCIP_RETCODE sepaHolesLp(SCIP *scip,              /**< SCIP data structure */
