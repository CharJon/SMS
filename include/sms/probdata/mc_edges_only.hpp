...
public:
    explicit ProbDataEdgesOnlyMc(Scip *scip, const Graph *g) : scip_(scip), mcGraph_(g) {
...
private:
    explicit ProbDataEdgesOnlyMc(Scip *scip, const Graph *g, std::vector<SCIP_VAR *> &edgeToVar,
                                 std::unordered_map<unsigned int, NetworKit::Edge> &varToEdge)
        : scip_(scip), mcGraph_(g), edgeIdToVar_(std::move(edgeToVar)), varToEdge_(std::move(varToEdge)) {}
...
public:
    ...
    NetworKit::Edge varIdToEdge(uint64_t varIndex) override { return varToEdge_[varIndex]; }
