...
public:
    explicit ProbDataRootedMc(Scip *scip, const Graph *g, NetworKit::node sun) : scip_(scip), mcGraph_(g), root_(sun) {
...
private:
    explicit ProbDataRootedMc(Scip *scip, const Graph *g, NetworKit::node sun, std::vector<SCIP_VAR *> &nodeToVar,
        ...
        : scip_(scip),
          mcGraph_(g),
          root_(sun),
          nodeToVar_(std::move(nodeToVar)),
          edgeIdToVar_(std::move(edgeToVar)),
          varToNode_(std::move(varToNode)),
          varToEdge_(std::move(varToEdge)) {}
...
public:
    ...
    SCIP_VAR *edgeIdToVar(NetworKit::index edgeId) const override { return edgeIdToVar_[edgeId]; }
    ...
    const Graph *getMcGraph() const override { return mcGraph_; }
    ...
    SCIP_RETCODE scip_delorig(SCIP * /*scip*/ /**< SCIP data structure */
                              ) override {
        return SCIP_OKAY;
    }
     ...
     */
    SCIP_RETCODE
    scip_trans(SCIP *scip, ObjProbData **objprobdata, /**< pointer to store the transformed problem data object */
        ...
        auto *transProbData = new ProbDataRootedMc(scip_, mcGraph_, root_, nodeToTransVar, edgeToTransVar,
