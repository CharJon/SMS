...
void buildRootMcModel(Scip **s, const Graph *g, NetworKit::node root, bool allBinary) {
    ...
    SCIP_CALL_EXC(SCIPcreateObjProb(scip, "mc root triangulated model", probdata, true))
    ...
    for (auto e : g->edgeRange()) {
        if (e.u != root && e.v != root) {
            SCIP_VAR *vars[3] = {probdata->edgeToVar(e.u, e.v), probdata->nodeToVar(e.u), probdata->nodeToVar(e.v)};
            const std::string name("xor_" + std::to_string(e.u) + "_" + std::to_string(e.v));
            addXorConstraintNoCapture(scip, vars, 3, name);
        }
    }
}