...
triangle sortTriangle(const std::vector<NetworKit::node> &inTriangle) {
...
fourcycle sortFourCycle(const std::vector<NetworKit::node> &cycle, const NetworKit::Graph &graph) {
    assert(cycle.size() == 4);
