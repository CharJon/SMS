...
#define CONSHDLR_NAME "cycleparity"
#define CONSHDLR_DESC "cycle parity constraints"
...
#define CONSHDLR_EAGERFREQ                                                                                             \
    100 /**< frequency for using all instead of only the useful constraints in separation,                             \
...
ConshdlrOscw::ConshdlrOscw(Scip *scip, const Graph *sepGraph)
...
SCIP_RETCODE SCIPcreateConsOddCyclesFast(
    ...
    /* create constraint */
    SCIP_CALL(SCIPcreateCons(scip, cons, name, conshdlr, consdata, initial, separate, enforce, check, propagate, local,
                             modifiable, dynamic, removable, FALSE));

    return SCIP_OKAY;
}

/** creates and captures an OddCycle-MaxCut constraint */
SCIP_RETCODE SCIPcreateConsBasicOddCyclesFast(SCIP *scip,        /**< SCIP data structure */
                                              SCIP_CONS **cons,  /**< pointer to hold the created constraint */
                                              const char *name,  /**< name of constraint */
                                              const Graph *graph /**< the graph to separate on */
) {
    return SCIPcreateConsOddCyclesFast(scip, cons, name, graph, FALSE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE,
                                       TRUE);
}
 ...
 */
SCIP_RETCODE
ConshdlrOscw::scip_check(SCIP *scip, SCIP_CONSHDLR * /*conshdlr*/, SCIP_CONS ** /*conss*/, int nconss, SCIP_SOL *sol,
                         SCIP_Bool /*checkintegrality*/, SCIP_Bool /*checklprows*/, SCIP_Bool /*printreason*/,
 ...
 */
SCIP_RETCODE
ConshdlrOscw::scip_sepasol(SCIP *scip, SCIP_CONSHDLR *conshdlr, SCIP_CONS **conss, int nconss, int nusefulconss,
...
SCIP_RETCODE ConshdlrOscw::sepaOddCycles(SCIP *scip, SCIP_CONSHDLR *conshdlr, SCIP_CONS **, int, int, SCIP_SOL *sol,
    ...
    *result = SCIP_DIDNOTFIND;
    ...
    auto *probData = SCIPgetObjProbData(scip);
    auto probDataMc = dynamic_cast<ProbDataRootedMc *>(probData);
    ...
    for (auto u : sepGraph_->nodeRange()) {
        bool blocklisted = u == probDataMc->getRoot() ? true : scipVarIsFixedLocal(scip, probDataMc->nodeToVar(u));
        ocwSeparator_.updateBlacklist(u, blocklisted);
    }

    // Separate from each node
    auto nodeRange = sepGraph_->nodeRange();
    for (auto it = nodeRange.begin(); it != nodeRange.end() && *result != SCIP_CUTOFF; ++it) {
        auto currentNode = *it;
        if (currentNode == probDataMc->getRoot() or scipVarIsFixedLocal(scip, probDataMc->nodeToVar(currentNode))) {
            continue;
        }
        ...
        for (auto &violatedOcw : violatedOcws) {
            ...
            unsigned int rhs = violatedOcw.numCrosses() - 1;

            SCIP_ROW *row;
            SCIP_CALL(SCIPcreateEmptyRowConshdlr(scip, &row, conshdlr, "osc", lhs, rhs, FALSE, FALSE, TRUE));

            SCIP_CALL(SCIPcacheRowExtensions(scip, row));
            for (unsigned int i = 0; i < violatedOcw.size(); i++) {
                auto u = violatedOcw.getIthNode(i);
                auto v = violatedOcw.getIthNode(i + 1);
                auto coefficient = violatedOcw.ithEdgeIsStay(i) ? -1. : 1.;
                SCIP_VAR *var = probDataMc->edgeToVar(u, v);
                // get value of variable in solution
                // auto val = SCIPgetSolVal(scip, nullptr, var);
                // std::cout << "var: " << SCIPvarGetName(var) << " val: " << val << " & ";
                SCIP_CALL(SCIPaddVarToRow(scip, row, var, coefficient));
            ...
            if (sepaCutpool_) {
                if (SCIPisCutEfficacious(scip, nullptr, row)) {
                    // SCIP_CALL(SCIPaddDelayedPoolCut(scip, row));
                    // get efficiency of cut
                    SCIP_Real eff = SCIPgetCutEfficacy(scip, nullptr, row);
                    // calculate the orthogonality of a cut
                    // SCIP_Real ortho = SCIProwGetOrthogonality(scip, nullptr, row);
...
SCIP_RETCODE
ConshdlrOscw::sepaOddCyclesLp(SCIP *scip,              /**< SCIP data structure */
...
SCIP_RETCODE ConshdlrOscw::scip_enfolp(SCIP *scip, SCIP_CONSHDLR *conshdlr, SCIP_CONS **conss, int nconss,
...
SCIP_RETCODE
ConshdlrOscw::scip_enfops(SCIP * /*scip*/, SCIP_CONSHDLR * /*conshdlr*/, SCIP_CONS ** /*conss*/, int /*nconss*/,
 ...
 */
SCIP_RETCODE
ConshdlrOscw::scip_lock(SCIP *scip, SCIP_CONSHDLR * /*conshdlr*/, SCIP_CONS * /*cons*/, SCIP_LOCKTYPE /*locktype*/,
