...
public:
    MaximalCliquesImpl(const NetworKit::Graph &G, std::function<bool(const std::vector<node> &)> &callback)
        : G(&G),
          callback(callback),
          pxvector(G.numberOfNodes()),
          pxlookup(G.upperNodeIdBound()),
          firstOut(G.upperNodeIdBound() + 1),
          head(G.numberOfEdges()) {}
...
private:
    ...
    template <typename F>
    void forOutEdgesOf(node u, F lambda) const {
...
public:
    void run() {
        ...
        for (index iu = orderedNodes.size(); (!stop) && iu-- > 0;) {
            ...
            std::vector<node> r = {u};
            stop = tomita(xpbound - xcount, xpbound, xpbound + pcount, r);
    ...
    // returns true if no more cliques shall be computed
    bool tomita(index xbound, index xpbound, index pbound, std::vector<node> &r) {
        auto stop = false;
...
MaximalCliques::MaximalCliques(const NetworKit::Graph &G,
...
void MaximalCliques::run() {
    ...
    MaximalCliquesImpl(*G, callback).run();
